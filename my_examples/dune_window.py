...
class WorldBuilder(gegede.builder.Builder):
    ...
    '''
    def configure(self, material = 'Air', show_cover_sheets = True, length = Q("8620cm") * 1.1 , height = Q("3800cm") * 1.1, depth = Q("374.62cm") *1.1,
                  n_support_beams = 8, n_support_beams_hori = 1, verti_active_area = Q("0mm"), hori_active_area = Q("0mm"),  **kwds):
        print ("Configure WorldBuilder for " + self.name)
        self.material, self.length, self.height, self.depth = (material, length, height, depth)
    ...
    def construct(self, geom):
        ...
        zaxis_smaller_rectangle_subBuilder = self.get_builders()[7] #to remove the wall in the zaxis
        #get the support beam
        support_beam_subBuilder = self.get_builders()[3]
        #get the triangle_builder
        middle_triangle_subBuilder = self.get_builders()[5]
        side_triangle_subBuilder = self.get_builders()[6]
        #get support beam end holes
        support_beam_hole_subBuilder = self.get_builders()[8]
        #get the sideways support beam
        sideways_support_beam_subBuilder = self.get_builders()[4]
        #get the line for the end of the support beam
        support_beam_endline_subBuilder = self.get_builders()[9]
        #get the extra thick intersection points
        support_beam_intersection_subBuilder = self.get_builders()[10]
        support_beam_hole_hori_subBuilder = self.get_builders()[11]

        #make the steel sheets
        inner_sheet_subBuilder = self.get_builders()[12]
        outer_sheet_subBuilder = self.get_builders()[13]

        #get hori triangles
        hori_middle_triangle_subBuilder = self.get_builders()[14]
        hori_side_triangle_subBuilder = self.get_builders()[15]
        ...
        if self.show_cover_sheets == True:
            ...
            support_beam_union_shape = geom.shapes.Boolean( inner_sheet_subBuilder.name+'_union_shape_' , type = inner_sheet_boolean ,
                                                       first = support_beam_union_shape, second= inner_sheet_shape, pos = inner_sheet_pos)
        else:
            pass
        

        support_beam_union_lv = geom.structure.Volume('vol'+ support_beam_union_shape.name, material=support_beam_subBuilder.material, shape=support_beam_union_shape)
...
class Block(gegede.builder.Builder):
    ...
    def configure(self, material = 'Steel', length = Q("5cm"), height = Q("1cm"), thickness = Q("0.1cm"),  **kwds):
