...
def make_line(box, ab, d, start):
    ...
    if ab == "XY":
        a = 0
        b = 1
        if d == "X": fa2 = opposite_value
        if d == "Y": fb2 = opposite_value
    if ab == "XZ":
        a = 0
        b = 2
        if d == "X": fa2 = opposite_value
        if d == "Z": fb2 = opposite_value
    if ab == "YZ":
        a = 1
        b = 2
        if d == "Y": fa2 = opposite_value
        if d == "Z": fb2 = opposite_value
    if ab == "YX":
        a = 1
        b = 0
        if d == "X": fb2 = opposite_value
        if d == "Y": fa2 = opposite_value
    if ab == "ZX":
        a = 2
        b = 0
        if d == "X": fb2 = opposite_value
        if d == "Z": fa2 = opposite_value
    if ab == "ZY":
        a = 2
        b = 1
        if d == "Y": fb2 = opposite_value
...
def draw_flux_window(max_z, image_name):
    ...
    rockbox = [(-4896.4*M,-661.473*M,-30.54*M), (621.1*M,1781.527*M,2954.9*M)]
    add_rockbox = True
    ...
    for num, line in enumerate(lines):
        color = ROOT.kRed
        if change_color != None and num >= change_color: color = ROOT.kCyan
...
def draw_histograms(input_file):
    ...
    for key in root_file.GetListOfKeys():
        ...
        if isinstance(obj, ROOT.TH2):
            ...
            if "(m)" in obj.GetXaxis().GetTitle(): obj.GetYaxis().SetTitle("N Vertices")
            obj.Draw()
            dunestyle.Simulation()
            #print(f"{obj.GetName()} integral: {obj.Integral()}")
            canvas.Print(os.path.join(output_subdir, image_name + ".png"))
