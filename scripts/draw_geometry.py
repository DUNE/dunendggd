...
def material_to_rgb(atomic_number, density, 
                    atomic_min=1, atomic_max=55.845,  # default for H to U
                    density_min=0.0, density_max=13.0, bw = False):  # approximate for materials
...
def material_is_air(node):
    ...
    out = False
    if material != None:
        if material.GetName() == "Air": out = True
    return out
...
def material_is_clear(node):
    ...
    if material != None:
        if material.GetName() == "Air": out = True
        if material.GetName() == "Vac": out = True
    return out
...
def color_function(node, highlight, exclude_clear = True):
    ...
    try: 
        ...
        color = None
        if exclude_clear and material_is_clear(node): color = white
        else:
            if highlight == "random":
                color = random_color(name)
            if highlight == "scintillator":
                if "scin" in name: color = black
                else: color = white
            if highlight == "material":
                material = get_material(node)
                if material == None: color = white
                else:
                    if material.GetName() == "Air": color = white
                    if material.GetName() == "Scintillator": color = yellow
                    if material.GetName() == "Polyurethane": color = yellow
                    if material.GetName() == "SteelTMS": color = blue
                    if "Steel" in material.GetName(): color = blue
                    if material.GetName() == "Rock": color = black
                    if color == None:
                        raise NotImplementedError(f"Do not have a color for {material.GetName()}")
            if highlight == "density" or highlight == "density_bw":
                ...
                if material == None: color = white
                    ...
                    a = material.GetA()
                    bw = False
                    if highlight == "density_bw": bw = True
                    if VERBOSE: print(f"atomic number: {a:0.2f},\tdensity: {density:0.2f},\tmaterial name: {material.GetName()}")
...
def calc_projection(i, j, matrix):
    ...
    x, y, z = transformed_vec.X() + matrix[0][3], transformed_vec.Y() + matrix[1][3], transformed_vec.Z() + matrix[2][3]
    return (x, y, z)
...
def create_projection_matrix(width, height, box_min, box_max, projection="XY"):
    ...
    if projection == "XY":
        ...
        scale[0][0] = dx 
        scale[1][1] = dz
        #temp = ROOT.TMatrixD(4, 4).AMultB(rotation, scale)
...
def draw_image(geom, matrix, w, h, color_function):
    ...
    print(f"time for {w}x{h} image was {t:0.2f}s, or {time_per_pixel*1000:0.3f}ms/pixel")
    return image
...
def get_axis(matrix, w, h, projection):
    ...
    nbins = 1000
    if VERBOSE: print(f"size: {w}x{h}\tx axis: {start_x:0.2f} to {end_x:0.2f},\ty axis: {start_y:0.2f} to {end_y:0.2f}")
    if projection == "XZ":
...
def unit_float(value):
    ...
    return number * UNIT_CONVERSIONS.get(unit, 1.0)  # Defaults to cm if no unit is provided
...
def get_color_function(color_function_name, exclude_clear):
    out = None
    if color_function_name == "scintillator": out = lambda x: color_function(x, "scintillator", exclude_clear)
    if color_function_name == "material":     out = lambda x: color_function(x, "material", exclude_clear)
    if color_function_name == "density_bw":   out = lambda x: color_function(x, "density_bw", exclude_clear)
    if color_function_name == "density":      out = lambda x: color_function(x, "density", exclude_clear)
    if color_function_name == "random":       out = lambda x: color_function(x, "random", exclude_clear)
    
    if out == None:
        raise ValueError(f"Did not understand color function named {color_function_name}")
    return out
    
def parse_args():
    global VERBOSE
    parser = argparse.ArgumentParser(description="Process geometry files with optional transformations and visualization settings.")

    parser.add_argument("--filename", required=True, help="Input geometry filename (required).")
    ...
    parser.add_argument("--axis", action='store_true', help="Whether to draw the axis (experimental, will make image more square)")
    parser.add_argument("--translate", "-t", type=unit_float, default=0, metavar="VALUE",
    ...
    parser.add_argument("--matrix", type=str, help="Custom 4x4 transformation matrix (16 space-separated values).")
    parser.add_argument("--verbose", action='store_true', help="Print interpreted matrix plus some test values to check")
    ...
    if not args.outfilename:
        args.outfilename = os.path.join(args.outdir, (args.view if args.view else "custom_matrix") + ".png")
    else:
...
if __name__ == "__main__":
    ...
    if args.axis:
        ...
        canvas = ROOT.TCanvas("c1", "c1", W, H)#int(args.width * 1.2), int(args.height * 1.2))
